...
class DrawingView(context: Context, attrs: AttributeSet) : View(context,attrs) {
    ...
    private var mBrushSize :
            Float = 0.toFloat()// A variable for stroke/brush size to draw on the canvas.
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when(event?.action){
            ...
            MotionEvent.ACTION_UP ->{
                Paths.add(mDrawPath!!)//Add when to stroke is drawn to canvas and added in the path arraylist.
                mDrawPath = CustomPath(color,mBrushSize)
            }
            else -> return false
        }
